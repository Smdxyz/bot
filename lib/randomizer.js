// --- START OF FILE lib/randomizer.js ---
...
/**
 * Generates a random person's full name and sex.
 * This is used for creating a consistent identity for automation.
 * @returns {{fullName: string, sex: 'male'|'female'}}
 */
...
/**
 * Generates a random plausible Indonesian address for billing forms.
 * @returns {object} An object containing address components.
 */
export const generateIndoAddress = () => {
    const city = faker.location.city();
    const state = faker.location.state();
  
    return {
      address1: `Jl. ${faker.location.street()} No. ${randInt(1, 150)}, Kel. ${faker.location.street()}, Kec. ${faker.location.city()}`,
      city: city,
      region: state,
