...
const setupSchema = () => {
    db.exec(`
        CREATE TABLE IF NOT EXISTS users (
            ...
            balance INTEGER DEFAULT 2500, -- Bonus awal 2500
...
export const getUser = (id, refCandidate = null) => {
    ...
    if (!user) {
        ...
        let balance = 2500; // Modal awal user baru
        let referrerId = null;
        ...
        if (refCandidate) {
            ...
            if (referrer && referrer.id !== id) {
                ...
                balance += 1000; // Bonus tambahan buat yang diundang
                
                // Bonus buat yang mengundang
...
// --- HELPER PRICE (VIP DISCOUNT) ---
export const calculatePrice = (userId, basePrice) => {
    const user = db.prepare('SELECT vip FROM users WHERE id = ?').get(userId);
    if (user && user.vip === 1) {
        return Math.ceil(basePrice * 0.5); // DISKON 50%
    }
    return basePrice;
};
